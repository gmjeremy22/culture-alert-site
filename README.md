...
- 암호 해제 후 첫 추천 화면의 검색창에서 바로 입력할 수 있으며, 상단 검색 버튼과 같은 통합 검색 결과창을 사용합니다.
...
- 기본 추천은 기관 규모를 `대표 기관 / 주요 기관 / 지역 핵심 기관 / 기타` 등급으로 먼저 나눈 뒤, 같은 등급 안에서 취향 적합성과 일정 품질을 반영합니다. 마감 임박은 주요 기관 등급을 뒤집지 못하며 별도 패널과 등급 내 보조 정렬로만 다룹니다.
...
- 매일 업데이트와 코드 변경 배포는 모두 마지막 성공 DB를 복원한 뒤 수집기를 실행합니다.
- 특정 기관 홈페이지가 일시적으로 응답하지 않아도 마지막 정상 일정은 DB에 남고, 날짜 상태 갱신으로 종료 여부를 관리합니다.
- 성공한 실행의 DB는 다음 실행용 GitHub Actions 캐시에 저장되며, 수집기별 발견·갱신·실패 건수는 Actions 로그에 출력됩니다.
- 성공 DB 캐시를 복원하지 못한 실행은 빈 DB로 일부 데이터만 배포하지 않고 즉시 중단합니다.
- 기관 홈페이지의 일시적인 시간 초과나 네트워크 오류는 같은 실행에서 한 번 자동 재시도합니다.
